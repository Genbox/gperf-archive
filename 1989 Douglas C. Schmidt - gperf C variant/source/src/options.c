...
usage ()
{ 
  report_error ("usage: %n [-acCdDf[num]gGhH<hashname>i<init>jk<keys>K<keyname>lnN<name>oprs<size>StTv].\n(type %n -h for help)\n");
}
...
\njump value = %d\nmax associcated value = %d\ninitial associated value = %d\n",
               ...
               option.key_name, option.jump, option.size - 1, option.initial_asso_value);
...
{ 
  ...
  while ((option_char = getopt (argc, argv, "adcCDf:gGhH:i:j:k:K:lnN:oprs:StTv")) != EOF) 
    {
      switch (option_char)
        {
            ...
            if (*optarg == '*') /* Use all the characters for hashing!!!! */
            ...
            else 
              {
                ...
                *key_pos = EOS;
                ...
                if (! (option.total_key_positions = (key_pos - option.key_positions))) 
                  report_error ("no keys selected\n%e%a", usage);
                else if (! key_sort (option.key_positions, option.total_key_positions))
                  report_error ("duplicate keys selected\n%e%a", usage);

                if (option.total_key_positions != 2 
